...
void chapter6_tcp_test(void)
{
    ...
    uv_ip4_addr("127.0.0.1", 0, &addr);  /* 端口0让系统自动分配 */
...
void chapter11_process_test(void)
{
    ...
    child_stdio[0].flags = UV_IGNORE;  /* stdin: 忽略 */
    child_stdio[1].flags = UV_CREATE_PIPE | UV_WRITABLE_PIPE;  /* stdout: 管道 */
    child_stdio[1].data.stream = (uv_stream_t *)&child_stdout_pipe;
    child_stdio[2].flags = UV_INHERIT_FD;  /* stderr: 继承 */
...
static void on_async(uv_async_t *handle)
{
    ...
    if (async_count >= 3) {
        uv_async_send(handle);  /* 发送最后一次通知后关闭 */
...
static ChapterEntry chapters[] = {
    {"1",  "事件循环",           chapter1_event_loop_test},
    {"2",  "句柄与请求",         chapter2_handle_request_test},
    {"3",  "定时器",             chapter3_timer_test},
    {"4",  "空闲/准备/检查",     chapter4_idle_prepare_check_test},
    {"5",  "文件系统",           chapter5_file_system_test},
    {"6",  "TCP网络",            chapter6_tcp_test},
    {"7",  "UDP网络",            chapter7_udp_test},
    {"8",  "管道",               chapter8_pipe_test},
    {"9",  "TTY",                chapter9_tty_test},
    {"10", "信号处理",           chapter10_signal_test},
    {"11", "进程管理",           chapter11_process_test},
    {"12", "异步通知",           chapter12_async_test},
    {"13", "线程池",             chapter13_thread_pool_test},
    {"14", "DNS解析",            chapter14_dns_test},
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        ...
        for (int i = 0; i < (int)(sizeof(chapters) / sizeof(chapters[0])); i++) {
            if (strcmp(chapters[i].number, chapter) == 0) {
                chapters[i].func();
                return 0;
            }
        }
